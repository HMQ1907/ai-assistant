...
export function buildTradingAnalysisPrompt(payload: AnalysisPayload): string {
  return [
    ...
    `Mandatory rules: confidence below ${tradingRules.minConfidence} means NO_TRADE. Risk reward below 1:${tradingRules.minRiskReward} means NO_TRADE. estimated_loss_if_sl_hit above ${payload.maxLossUsdPerTrade} USD means NO_TRADE. If spread is available and too wide, or the market is choppy, return NO_TRADE.`,
    ...
    "Treat missing or invalid bid/ask as a warning and confidence penalty, not an automatic NO_TRADE. Do not invent bid, ask or spread.",
