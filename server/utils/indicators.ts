...
export function atr(candles: Candle[], period = 14): number {
  ...
  return round(trs.reduce((sum, value) => sum + value, 0) / Math.max(trs.length, 1))
}

export function supportResistance(candles: Candle[]): { support: number; resistance: number; swingHigh: number; swingLow: number } {
